...
import { LocationIcon, TemperatureIcon, SettingsIcon } from "../icons";
import { router, useRouter } from "expo-router";
import { getAuth, onAuthStateChanged, signOut } from "firebase/auth";
import { app } from "../../firebase/firebaseConfig";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { Svg, Path } from "react-native-svg";
import { styles as styles } from "@/styles/index";
import {
  FETCH_USER_CITY,
  KEY_WEATHER,
  FETCH_ITEMS,
  FETCH_LATEST_OUTFITS,
} from "@/config";
...
export default function HomeScreen() {
  ...
  const fetchItems = async () => {
    const token = await getToken();
    try {
      const response = await fetch(FETCH_ITEMS, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          <Text style={styles.headerTitle}>Weather today</Text>
        </View>
        ...
        <Text style={styles.sectionTitle}>Latest outfits</Text>
        <View style={styles.carouselSectionOutfit}>
          {loadingOutfits ? (
            ...
            <View style={styles.noOutfitsContainer}>
              <Text>No outfit sets found</Text>
            </View>
          ) : (
            <FlatList
              data={outfitSets}
